...
sub new {
  ...
  Baruwa::Scanner::Log::WarnLog("Illegal value for Read IP Address From Received Header, should be a number") unless $getipfromheader =~ /^\d+$/;
  ...
  my $usewatermark = (Baruwa::Scanner::Config::Value('usewatermarking', $this) =~ /1/)?1:0;
  ...
  if (($usewatermark) && (length $mshmackey)) {
    ...
    $chkmshmacskip = Baruwa::Scanner::Config::Value('checkmshmacskip', $this);
    ...
    my $mshmacheader = Baruwa::Scanner::Config::Value('mshmacheader', $this);
    ...
    if ($chkmshmacnull && $this->{fromuser} eq "" && $this->{clientip} ne '0.0.0.0') {
...
sub WriteHeaderFile {
  ...
  Baruwa::Scanner::Lock::openlock($header, ">$filename", "w") or Baruwa::Scanner::Log::DieLog("Cannot create + lock headers file %s, %s", $filename, $!);
  ...
  $filename =~ /^(.*)$/;
  $filename = $1;
  chown $global::MS->{work}->{uid}, $global::MS->{work}->{gid}, $filename if $global::MS->{work}->{changeowner};
}
...
sub IsSpam {
  ...
  my $LocalNotSpamText = Baruwa::Scanner::Config::LanguageValue($this, 'notspam');
  ...
  $includesaheader = Baruwa::Scanner::Config::Value('includespamheader', $this);
  ...
  if ($this->{ret}) {
   Baruwa::Scanner::Log::InfoLog("Valid RET hash found in Message %s, skipping Spam Checks",$this->{id});
   return 0;
  }

  # Skip Spam Checks if Watermark is valid
  if ($this->{mshmacskipvalid}) {
    Baruwa::Scanner::Log::InfoLog("Valid Watermark HASH found in Message %s Header, skipping Spam Checks", $this->{id}); 
    return 0;
  }

  if ($this->{mshmacnullpresent} && $this->{mshmacnullvalid}) {
    Baruwa::Scanner::Log::InfoLog("Message %s from %s has valid watermark", $this->{id}, $this->{clientip});
  } elsif ($this->{mshmacnullpresent} && $this->{mshmacnullvalid}==0) {
    # If the sender is empty then treat unmarked messages as spam perhaps?
    my $mshmacnull = lc(Baruwa::Scanner::Config::Value('mshmacnull', $this));
    ...
    if ($mshmacnull =~ /delete/) {
      $this->{deleted} = 1;
      $this->{dontdeliver} = 1;
      Baruwa::Scanner::Log::InfoLog("Message %s from %s has no (or invalid) watermark or sender address, deleted", $this->{id}, $this->{clientip}) if $LogSpam;
    }
    elsif ($mshmacnull =~ /high/) {
      my $highscore = Baruwa::Scanner::Config::Value('highspamassassinscore', $this);
      $this->{isspam} = 1;
      $this->{ishigh} = 1;
      $this->{sascore} = $highscore if $this->{sascore} < $highscore;
      $this->{spamreport} = $LocalSpamText . "(no watermark or sender address)";
      Baruwa::Scanner::Log::InfoLog("Message %s from %s has no (or invalid) watermark or sender address, marked as high-scoring spam", $this->{id}, $this->{clientip}) if $LogSpam;
      return 1;
    } 
    elsif ($mshmacnull =~ /spam/) {
      my $reqscore = Baruwa::Scanner::Config::Value('reqspamassassinscore', $this);
      $this->{isspam} = 1;
      $this->{sascore} = $reqscore if $this->{sascore} < $reqscore;
      $this->{spamreport} = $LocalSpamText . "(no watermark or sender address)";
      Baruwa::Scanner::Log::InfoLog("Message %s from %s has no (or invalid) watermark or sender address, marked as spam", $this->{id}, $this->{clientip}) if $LogSpam;
      return 1;
    }
    # spam/high/normal can also be a number, which is added to the Spam Score
      ...
      Baruwa::Scanner::Log::InfoLog("Message %s had bad watermark, added %s to spam score", $this->{id}, $mshmacnull+0.0) if $LogSpam;
      my($mshspam, $mshhigh) = Baruwa::Scanner::SA::SATest_spam($this, 0.0, $this->{sascore}+0.0);
      ...
      Baruwa::Scanner::Log::InfoLog("Message %s from %s has no (or invalid) watermark or sender address", $this->{id}, $this->{clientip});
    }
  }
  ...
  if ($recipientcount<=$maxrecips) {
    if (Baruwa::Scanner::Config::Value('spamwhitelist', $this)) {
      ...
      Baruwa::Scanner::Log::InfoLog("Message %s from %s (%s) is whitelisted", $this->{id}, $this->{clientip}, $this->{from}) if $LogSpam || $LogNonSpam;
      $iswhitelisted = 1;
    ...
    Baruwa::Scanner::Log::InfoLog("Message %s from %s (%s) ignored whitelist, " .
                              "had %d recipients (>%d)", $this->{id},
                              $this->{clientip}, $this->{from},
                              $recipientcount, $maxrecips) if $LogSpam || $LogNonSpam;
  }

  # If it's a blacklisted address, don't bother doing any checks at all
  if (!$iswhitelisted && Baruwa::Scanner::Config::Value('spamblacklist', $this)) {
    $this->{spamblacklisted} = 1;
    $this->{isspam} = 1;
    $this->{ishigh} = 1 if Baruwa::Scanner::Config::Value('blacklistedishigh', $this);
    $this->{spamreport} = $LocalSpamText . ' (' . Baruwa::Scanner::Config::LanguageValue($this, 'blacklisted') . ')';
    Baruwa::Scanner::Log::InfoLog("Message %s from %s (%s) to %s" .
                              " is spam (blacklisted)",
                              $this->{id}, $this->{clientip},
                              $this->{from}, $todomain) if $LogSpam;
    return 1;
  }

  my $whitelistreport = '';
  if ($iswhitelisted) {
    $whitelistreport = ' (' . Baruwa::Scanner::Config::LanguageValue($this, 'whitelisted') . ')';
  }
  ...
  if ($this->{size} > $maxtestsize) {
    $this->{spamreport} = Baruwa::Scanner::Config::LanguageValue($this, 'skippedastoobig');
    $this->{spamreport} = $this->ReflowHeader(Baruwa::Scanner::Config::Value('spamheader',$this), $this->{spamreport});
    Baruwa::Scanner::Log::InfoLog("Message %s from %s (%s) to %s is too big for spam checks (%d > %d bytes)",
                              $this->{id}, $this->{clientip},
                              $this->{from}, $todomain,
  ...
  if (!$iswhitelisted) {
    ...
    my $highrblthreshold = Baruwa::Scanner::Config::Value('highrbls', $this);
  ...
  if ($usegsscanner) {
    ...
    Baruwa::Scanner::Log::InfoLog("Custom Spam Scanner for message %s from %s " .
                              "(%s) to %s report is %s %s",
                              $this->{id}, $this->{clientip},
                              $this->{from}, $todomain, $gsscore, $gsreport) if $LogSpam && ($gsscore!=0 || $gsreport ne "");
  }
  ...
  unless (Baruwa::Scanner::Config::Value('usespamassassin', $this)) {
    ...
    $this->{isspam} = 1 if $this->{sascore}+0.0 >= Baruwa::Scanner::Config::Value('reqspamassassinscore',$this)+0.0;
    $this->{ishigh} = 1 if $this->{sascore}+0.0 >= Baruwa::Scanner::Config::Value('highspamassassinscore',$this)+0.0;
    Baruwa::Scanner::Log::InfoLog("Message %s from %s (%s) to %s is %s",
                              $this->{id}, $this->{clientip},
                              $this->{from}, $todomain, $rblspamheader) if $RBLsaysspam && $LogSpam;
  ...
  if ($this->{isspam} && !Baruwa::Scanner::Config::Value('checksaifonspamlist', $this)) {
    $this->{spamwhitelisted} = $iswhitelisted;
    Baruwa::Scanner::Log::InfoLog("Message %s from %s (%s) to %s is %s",
                              $this->{id}, $this->{clientip},
                              $this->{from}, $todomain, $rblspamheader) if $RBLsaysspam && $LogSpam;
  ...
  ($SAsaysspam, $SAHighScoring) = Baruwa::Scanner::SA::SATest_spam($this, $this->{sascore}+0.0, $sascore+0.0);
  ...
  $saheader = Baruwa::Scanner::Config::LanguageValue($this, 'spamassassin') .
...
sub HandleHamAndSpam {
  ...
  if ($HamSpam eq 'nonspam') {
    ...
    if ($this->{ishigh}) {
      ...
      $actions = Baruwa::Scanner::Config::Value('highscorespamactions', $this);
    } else {
  ...
  while ($actions =~ s/\"([^\"]+)\"//) {
    ...
    Baruwa::Scanner::Log::WarnLog("Syntax error in \"header\" action in spam " .
  ...
  if ($#actions==0 && $actions[0] =~ /\// && $actions[0] !~ /^store-\//) {
    Baruwa::Scanner::Log::WarnLog('Your spam actions "%s" looks like a filename.' .
  ...
  if ($sarule) {
    ...
    foreach my $looprule ((keys %{$this->{sarules}}), @sascorerules) {
      ...
      foreach $action (split(/\0+/,$sarule{$looprule})) {
        ...
        Baruwa::Scanner::Log::InfoLog("SpamAssassin Rule Actions: rule %s caused action %s in message %s", $looprule, $action, $this->{id}) if $logsaactions;
        if ($action !~ /^notify/ && $action =~ s/^no\w?\W*//) { # Anything started no, not not, etc.
          ...
          if ($action =~ /\@/) {
            ...
            Baruwa::Scanner::Log::WarnLog("Syntax error in \"header\" action in " .
  ...
  if ($actions{'deliver'} || $actions{'forward'}) {
    ...
    Baruwa::Scanner::Log::NoticeLog("Spam Actions: message %s actions are %s",
  ...
  if ($actions{'bounce'}) {
    if ($HamSpam eq 'nonspam') {
      Baruwa::Scanner::Log::WarnLog("Does not make sense to bounce non-spam");
    } else {
      #Baruwa::Scanner::Log::WarnLog('The "bounce" Spam Action no longer exists');
      if ($this->{ishigh}) {
        Baruwa::Scanner::Log::NoticeLog("Will not bounce high-scoring spam")
      } else {
  ...
  if ($actions{'notify'}) {
    if ($HamSpam eq 'nonspam') {
      Baruwa::Scanner::Log::WarnLog("Does not make sense to notify recipient about non-spam");
    } else {
...
sub HandleSpamBounce {
  ...
  if ($this->{isrblspam} && !$this->{issaspam}) {
    $filename = Baruwa::Scanner::Config::Value('senderrblspamreport', $this);
    Baruwa::Scanner::Log::NoticeLog("Spam Actions: (RBL) Bounce to %s", $from)
    ...
    Baruwa::Scanner::Log::NoticeLog("Spam Actions: (SpamAssassin) Bounce to %s",
                              $from)
      if Baruwa::Scanner::Config::Value('logspam');
  }
  if ($filename eq "") {
    $filename = Baruwa::Scanner::Config::Value('senderbothspamreport', $this);
    Baruwa::Scanner::Log::NoticeLog("Spam Actions: (RBL,SpamAssassin) Bounce to %s",
                              $from)
      if Baruwa::Scanner::Config::Value('logspam');
  }
  ...
  if (Baruwa::Scanner::Config::Value('bouncespamasattachment', $this)) {
    ...
    $global::MS->{mta}->SendMessageString($this, $emailmsg, '<>')
      or Baruwa::Scanner::Log::WarnLog("Could not send sender spam bounce, %s", $!);
  }
}
...
sub HandleSpamBounceAttachment {
  ...
  push(@original, "\n");
  $this->{store}->ReadBody(\@original, Baruwa::Scanner::Config::Value(
                                          'maxspamassassinsize'));
...
sub RejectMessage {
  ...
  # sender address is "<>" so that it can't be bounced.
  $global::MS->{mta}->SendMessageString($this, $emailmsg, '<>')
    or Baruwa::Scanner::Log::WarnLog("Could not send rejection report for %s, %s",
                                 $id, $!);
  $this->{deleted} = 1;
  $this->{dontdeliver} = 1;
...
sub DeliverUnscanned {
  ...
  my(@removeme, $remove);
  @removeme = split(/[,\s]+/, Baruwa::Scanner::Config::Value('removeheaders', $this));
  ...
  if (Baruwa::Scanner::Config::Value('spamscorenotstars', $this)) {
    ...
    $stars = Baruwa::Scanner::Config::Value('spamstarscharacter') x $starcount;
  }
  if (Baruwa::Scanner::Config::Value('spamstars', $this) =~ /1/ && $starcount>0) {
  ...
  if ($this->{messagedisarmed}) {
    ...
    my $where = Baruwa::Scanner::Config::Value('disarmmodifysubject',$this);
  ...
  if ($this->{isspam} && $this->{ishigh}) {
    my $where = Baruwa::Scanner::Config::Value('highspammodifysubject',$this);
    if ($where =~ /end/ && !$global::MS->{mta}->TextEndsHeader($this, 'Subject:', $spamtag)) {
      $global::MS->{mta}->AppendHeader($this, 'Subject:', $spamtag, ' ');
    } elsif ($where =~ /start|1/ && !$global::MS->{mta}->TextStartsHeader($this, 'Subject:', $spamtag)) {
      $global::MS->{mta}->PrependHeader($this, 'Subject:', $spamtag, ' ');
...
sub AddFromAndTo {
  ...
  if (Baruwa::Scanner::Config::Value('addenvfrom', $this) !~ /0/) {
    ...
    $global::MS->{mta}->ReplaceHeader($this,
                        Baruwa::Scanner::Config::Value('envfromheader', $this),
                        $from);
  }
...
sub ZipAttachments {
  ...
  return unless Baruwa::Scanner::Config::Value('zipattachments', $this) =~ /1/;
  ...
  unless ($dir) {
    Baruwa::Scanner::Log::WarnLog("Error: could not open message dir %s", $explodeinto);
    return;
  }
  ...
  return if $totalsize < Baruwa::Scanner::Config::Value('attachzipminsize', $this);
  ...
  unless($result == AZ_OK) {
    ...
    Baruwa::Scanner::Log::WarnLog("Zip file %s for message %s could not be created",
                              $safezipname, $this->{id});
    return;
  }
...
sub Explode {
  ...
  if (!$entity && !MIME::Entity::BaruwaCounter()>=$maxparts) {
    unless ($this->{dpath}) {
      ...
      Baruwa::Scanner::Log::WarnLog("Failed to create message structures for %s" .
    ...
    Baruwa::Scanner::Log::WarnLog("Cannot parse " . $this->{headerspath} . " and " .
                 $this->{dpath} . ", $@");
  ...
  if ($maxparts>0 && MIME::Entity::BaruwaCounter()>=$maxparts) {
    ...
    Baruwa::Scanner::Log::WarnLog("Too many attachments (%d) in %s",
                            MIME::Entity::BaruwaCounter(), $this->{id});
    $this->{entity} = $entity; # In case it failed due to too many attachments
  ...
  my $DeleteTNEF = 0;
  $DeleteTNEF = 1 if Baruwa::Scanner::Config::Value('replacetnef', $this) !~ /[01]/;
  ...
  if (Baruwa::Scanner::Config::Value('tnefexpander') && $this->{tnefentity}) {
    ...
    $outputdir->open($explodeinto)
      or Baruwa::Scanner::Log::WarnLog("Failed to open dir " . $explodeinto .
    ...
    foreach $tneffile (@tneffiles) {
      ...
      $result = Baruwa::Scanner::TNEF::Decoder($explodeinto, $tneffile, $this);
      if ($result) {
        ...
        if ($DeleteTNEF && $this->{foundtnefattachments}) {
          ...
          Baruwa::Scanner::Log::InfoLog("Message %s has had TNEF %s removed",
                                    $this->{id}, $tnefnotype);
        }
      } else {
        Baruwa::Scanner::Log::WarnLog("Corrupt TNEF %s that cannot be " .
  ...
  unless(chdir $explodeinto) { # TAINT
    Baruwa::Scanner::Log::WarnLog("Could not chdir to %s just before unpacking " .
  ...
  if (defined($entity) && $entity->head) {
    if ($entity->is_multipart || $entity->head->mime_type =~ /^multipart/i) {
      ...
      if ($boundary eq "" || $boundary eq "\"\"" || $boundary =~ /\s$/) {
        my $cantparse = Baruwa::Scanner::Config::LanguageValue($this,
                                                           'cantanalyze');
  ...
  $couldnotreadmesg = Baruwa::Scanner::Config::LanguageValue($this,
                                                         'unreadablearchive');
  $passwordedmesg = Baruwa::Scanner::Config::LanguageValue($this,
                                                       'passwordedarchive');
  $nonpasswordedmesg = Baruwa::Scanner::Config::LanguageValue($this,
                                                       'nonpasswordedarchive');
  $toodeepmesg    = Baruwa::Scanner::Config::LanguageValue($this,
                                                       'archivetoodeep');
  ...
  if (Baruwa::Scanner::Config::Value('addtextofdoc', $this) =~ /1/) {
    ...
    my %nullhash = ();
    my $docfiles = Baruwa::Scanner::Antiword::FindDocFiles($this->{entity},$this->{entity},\%nullhash);
    ...
    #foreach my $docfile (@docfiles) {
      ...
      Baruwa::Scanner::Antiword::RunAntiword($explodeinto,$docfile,$parent,$this);
    }
  }
  ...
  if ($df) {
    ...
    if (defined($freek) && $freek<100 && $freek>=0) {
      Baruwa::Scanner::Log::WarnLog("Message %s is too big for available disk space in %s, skipping it", $this->{id}, $dir);
      my @toclear = ( $this->{id} );
      $workarea->ClearIds(\@toclear); # Delete attachments we might have made
      $this->DropFromBatch();
      return;
...
sub ExplodePartAndArchives {
  ...
  $create0files = 0;
  $create0files = 1 if Baruwa::Scanner::Config::Value('checkppafilenames', $this) =~ /1/;

  $silentviruses = ' '. Baruwa::Scanner::Config::Value('silentviruses', $this) .' ';
  $noisyviruses = ' ' . Baruwa::Scanner::Config::Value('noisyviruses', $this) .' ';
  ...
  OUTER: while($cyclecounter<30) {
    ...
    foreach $part (@parts) {
      ...
      my $uudfilename = "";
      $uudfilename = FindUUEncodedFile($file) if Baruwa::Scanner::Config::Value('lookforuu', $this) =~ /1/;
      #$file->close;
      $failisokay = 0;
      if ($buffer =~ /^MZ/) {
        $failisokay = 1;
      ...
      $file->close, next unless Baruwa::Scanner::Config::Value('findarchivesbycontent', $this) || $part =~ /\.(tar\.g?z|taz|tgz|tz|zip|exe|rar|uu|uue|doc|xls|ppt|dot|xlt|pps)$/i;
      $foundnewfiles = 1;
      ...
      if ($ziperror eq "nonpassword" || $rarerror eq "nonpassword") {
        ...
        my $f = substr($part,1);
        Baruwa::Scanner::Log::WarnLog("Non-password-protected archive (%s) in %s",
                                  $f, $this->{id});
        ...
        $this->{silent}=1 if $silentviruses =~ /\sZip-NonPassword|All-Viruses\s/i;
        $this->{noisy} =1 if $noisyviruses  =~ /\sZip-NonPassword\s/i;
      } elsif ($ziperror eq "password" || $rarerror eq "password") {
        # Trim off leading type indicator character for logging.
        my $f = substr($part,1);
        Baruwa::Scanner::Log::WarnLog("Password-protected archive (%s) in %s",
                                  $f, $this->{id});
        ...
        $this->{otherinfected} = 1;
        $this->{cantdisinfect} = 1; # Don't even think about disinfecting this!
        $this->{silent}=1 if $silentviruses =~ /\sZip-Password|All-Viruses\s/i;
  ...
  if ($level>$maxlevels && !$onlycheckencryption && $maxlevels) {
    Baruwa::Scanner::Log::WarnLog("Files hidden in very deeply nested archive " .
                              "in %s", $this->{id});
...
sub UnpackUUE {
  ...
  Baruwa::Scanner::Log::InfoLog("Unpacking UU-encoded file %s to %s in message %s",
  ...
  unless ($out->open("> $safename")) {
    Baruwa::Scanner::Log::WarnLog("Unpacking UU-encoded file %s, could not create target file %s in message %s", $this->MakeNameSafe($uuencoded,$explodeinto), $safename, $this->{id});
    return;
  }
...
sub UnpackRar {
  ...
  foreach $member2 (@members) {
    ...
    if ($check =~ /\bEncrypted file:\s.+\(password incorrect/si) {
      ...
      if ($insistpasswords) {
        Baruwa::Scanner::Log::WarnLog("Non-Password Protected RAR Found");
        return "nonpassword";
      }
    }
    ...
    if (!$IsEncrypted && !$HasErrors) {
      ...
      unless ("$?" == 0 && $PipeReturn ne 'COMMAND_TIMED_OUT'){
        ...
        Baruwa::Scanner::Log::WarnLog("UnPackRar: Could not rename or use " .
    ...
    unless ("$?" == 0 && !$HasErrors && !$IsEncrypted && $PipeReturn ne 'COMMAND_TIMED_OUT') {
      # If we got an error, or this file is encrypted create a zero-length
      # file so the filename tests will still work.
      Baruwa::Scanner::Log::WarnLog("Unrar : Encrypted Or Extract Error Creating" .
                                " 0 length %s",$NameTwo);
...
sub SafePipe {
  ...
  eval {
    ...
    if ($pid) {
      ...
      exec map { m/(.*)/ } @args
        or Baruwa::Scanner::Log::WarnLog("SafePipe :  failed to execute %s", $Cmd);
  ...
  Baruwa::Scanner::Log::WarnLog("SafePipe in Message.pm : $Cmd failed with real error: $@")
  ...
  Baruwa::Scanner::Log::WarnLog("Safepipe in Message.pm : %s timed out!", $Cmd);
...
sub UnpackOle {
  ...
  if ($@) {
    ...
    Baruwa::Scanner::Log::WarnLog("Skipping OLE document unpacking due to OLE analysis failure");
  } else {
...
sub CreateEntitiesHelpers {
  ...
  MIME::WordDecoder->default->handler('*' => \&Baruwa::Scanner::Message::WordDecoderKeep7Bit);
...
sub Clean {
  ...
  while(($file, $text) = each %{$this->{allreports}}) {
    ...
    if ($type =~ /v/i) {
      ...
      if ($storeme) {
        $filename = Baruwa::Scanner::Config::Value('storedvirusmessage',
                                               $this);
      } else {
        $filename = Baruwa::Scanner::Config::Value('deletedvirusmessage',
                                               $this);
      }
    } elsif ($type =~ /f/i) {
      # It's a filename trap. Either delete or store it.
      if ($storeme) {
        $filename = Baruwa::Scanner::Config::Value('storedfilenamemessage',
                                               $this);
      } else {
        $filename = Baruwa::Scanner::Config::Value('deletedfilenamemessage',
                                               $this);
      }
    } elsif ($type =~ /c/i) {
      # It's dangerous content, either delete or store it.
      if ($storeme) {
        $filename = Baruwa::Scanner::Config::Value('storedcontentmessage',
                                               $this);
      } else {
        $filename = Baruwa::Scanner::Config::Value('deletedcontentmessage',
                                               $this);
      }
    } elsif ($type =~ /s/i) {
      # It's dangerous content, either delete or store it.
      if ($storeme) {
        $filename = Baruwa::Scanner::Config::Value('storedsizemessage',
                                               $this);
      } else {
        $filename = Baruwa::Scanner::Config::Value('deletedsizemessage',
                                               $this);
      }
    } elsif ($type eq 'm') {
  ...
  while(($entity, $text) = each %{$this->{entityreports}}) {
    ...
    if ($storeme) {
      $filename = Baruwa::Scanner::Config::Value('storedvirusmessage', $this);
    } else {
      $filename = Baruwa::Scanner::Config::Value('deletedvirusmessage', $this);
    }
...
sub CleanEntity {
  ...
  $warningfile = Baruwa::Scanner::Config::Value('attachmentwarningfilename', $this);
  ...
  unless ($parent) {
    ...
    if ($entity->bodyhandle) {
      ...
      $filename = Baruwa::Scanner::Config::Value('attachmentwarningfilename',
                                             $this);
  ...
  if ($Warning ne "") {
    $Disposition = Baruwa::Scanner::Config::Value('warningisattachment',$this)
                   ?'attachment':'inline';
...
sub ConstructWarning {
  ...
  my $localpostmaster = Baruwa::Scanner::Config::Value('localpostmaster', $this);
  my $postmastername  = Baruwa::Scanner::Config::LanguageValue($this, 'baruwa');
...
sub ReadVirusWarning {
  ...
  my $viruswarningname = Baruwa::Scanner::Config::Value('attachmentwarningfilename',
                                                    $this);
  my($line);
  ...
  my $fh = new FileHandle;
  $fh->open($file)
    or (Baruwa::Scanner::Log::WarnLog("Could not open inline file %s, %s",
                                  $file, $!),
        return undef);
  ...
  while (($typedattach, $text) = each %{$this->{allreports}}) {
    ...
    if ($this->{entity} eq $entity) {
      $infected{Baruwa::Scanner::Config::LanguageValue($this, "theentiremessage")}
        = 1;
    } else {
      $infected{"$attach"} = 1;
    }
  ...
  while (($typedattach, $text) = each %{$this->{entityreports}}) {
    $infected{Baruwa::Scanner::Config::LanguageValue($this, 'notnamed')} = 1;
  }
  $attach = substr($typedattach,1);
...
sub AppendSignCleanEntity {
  ...
  if (Baruwa::Scanner::Config::Value('attachimage', $this) =~ /1/ && !$this->{sigimagepresent}) {
    #print STDERR "Adding image signature\n";
    my $attach = Baruwa::Scanner::Config::Value('attachimagetohtmlonly', $this);
    if (($html && $attach =~ /1/) || $attach =~ /0/) {
      my $filename = Baruwa::Scanner::Config::Value('attachimagename', $this);  
      my $ext = 'unknown';
      $ext = $1 if $filename =~ /\.([a-z]{3,4})$/;
      $ext = 'jpeg' if $ext =~ /jpg/i;
      my $internalname =  Baruwa::Scanner::Config::Value('attachimageinternalname', $this);
...
sub DeliverUnmodifiedBody {
  ...
  my $subjectchanged = 0;
  my $disarmtag = Baruwa::Scanner::Config::Value('disarmsubjecttext',$this);
  my $phishingtag = Baruwa::Scanner::Config::Value('phishingsubjecttag', $this);
...
sub DeliverModifiedBody {
  ...
  my $contenttag = Baruwa::Scanner::Config::Value('contentsubjecttext', $this);
  ...
  if ($this->{nameinfected} &&   # Triggered a filename trap
    ...
    my $where = Baruwa::Scanner::Config::Value('contentmodifysubject',$this);
    ...
    # already been modified by another of your Baruwas.
    my $virustag = Baruwa::Scanner::Config::Value('virussubjecttext', $this);
    #print STDERR "I am infected\n" if $this->{infected};

    if ($this->{infected}) {
      my $where = Baruwa::Scanner::Config::Value('virusmodifysubject',$this);
...
sub FindSilentAndNoisyInfections {
  ...
  my(@silentin) = split(" ",Baruwa::Scanner::Config::Value('silentviruses', $this));
  my($silent, $silentin, @silent, $regexp, $allreports, $logstring, $allsilent);
  my($virusreports);

  my(@noisyin) = split(" ",Baruwa::Scanner::Config::Value('noisyviruses', $this));
...
sub WarnSender {
  ...
  if ($type =~ /v/i) {
    ...
    $msgname = Baruwa::Scanner::Config::Value('senderfilenamereport', $this);
  } elsif ($type =~ /e/i) {
...
sub CreatePostmasterNotice {
  ...
  if (Baruwa::Scanner::Config::Value('noticefullheaders', $this)) {
    ...
    $result .= Baruwa::Scanner::Config::LanguageValue($this, 'fullheadersare') .
               ":\n\n $headers\n\n";
  }
...
sub DeliverFiles {
  ...
  $newsubject = Baruwa::Scanner::Config::LanguageValue($this, 'disinfected') .
  ...
  $textfh->open($textfile)
    or Baruwa::Scanner::Log::WarnLog("Cannot open disinfected report message " .
                                 "file %s, %s", $textfile, $!);
  $output = "";
  my $line;
  my $ea = qr/([\(\)\[\]\.\?\*\+\^"'@<>:])/;
...
sub ArchiveToFilesystem {
  ...
  my $assumeisdir = (Baruwa::Scanner::Config::Value("assumeisdir", $this)
                     =~ /1/)?1:0;
  ...
  foreach $dir (@{$this->{archiveplaces}}) {
    ...
    foreach my $recip (0..$numrecips-1) {
      ...
      unless (-d "$target") {
        umask $global::MS->{quar}->{dirumask};
        mkpath "$target" or
          Baruwa::Scanner::Log::WarnLog("Cannot create directory %s", $target);
        umask 0077;
      }
...
sub AppendToMbox {
  ...
  if ($fh) {
    ...
    Baruwa::Scanner::Log::WarnLog("Failed to append message to pre-existing " .
                              "mbox file %s", $mbox);
  }
}
...
sub DisarmHTML {
  ...
  Baruwa::Scanner::Log::InfoLog('Content Checks: Detected and have disarmed ' .
...
sub EncapsulateMessage {
  ...
  $messagefh->open($filename)
    or Baruwa::Scanner::Log::WarnLog("Cannot open inline spam warning file %s, %s",
                                 $filename, $!);
  $emailmsg = "";
  while(<$messagefh>) {
    chomp;
...
sub HTMLToText {
  ...
  if ($entity->head->mime_attr('content-disposition') !~ /attachment/i &&
    ...
    Baruwa::Scanner::Log::InfoLog('Content Checks: Detected and will convert ' .
...
sub DisarmHTMLEntity {
  ...
  my $pipe = new IO::Pipe
    or Baruwa::Scanner::Log::DieLog('Failed to create pipe, %s, while parsing ' .
  ...
  if ($pid == 0) {
    ...
    unless ($outfh->open(">$newname")) {
      Baruwa::Scanner::Log::WarnLog('Could not create disarmed HTML file %s',
                                $newname);
      exit 1;
    }
    select $outfh;
    if ($DisarmPhishing) {
      HTML::Parser->new(api_version => 3,
        ...
        ->parse_file($oldname)
        or Baruwa::Scanner::Log::WarnLog("HTML disarming, can't open file %s: %s",
                                     $oldname, $!);
  ...
  if ($PipeReturn) {
    ...
    my $report = "Baruwa was attacked by a Denial Of Service attack, and has therefore
    ...
    my $report2 = Baruwa::Scanner::Config::LanguageValue(0, 'htmlparserattack');
...
sub DisarmTagCallback {
  ...
  if ($tagname eq 'form' && $DisarmFormTag) {
    ...
    $SigImageFound = 1
      if exists $attr->{'alt'} && $attr->{'alt'} =~ /Baruwa/i
...
sub DisarmEndtagCallback {
  ...
  if ($tagname eq 'iframe' && $DisarmIframeTag) {
    ...
    if ($alarm ||
      ...
      my $possiblefraudstart = Baruwa::Scanner::Config::LanguageValue(0, 'possiblefraudstart');
      ...
      if (InPhishingBlacklist($linkurl) and not InPhishingWhitelist($linkurl)) {
        ...
        print Baruwa::Scanner::Config::LanguageValue(0, 'definitefraudstart') .
              ' "' . $linkurl . '"' .
              Baruwa::Scanner::Config::LanguageValue(0, 'definitefraudend') .
        ...
        Baruwa::Scanner::Log::NoticeLog('Found definite phishing fraud from %s ' .
      ...
      if (!$StrictPhishing) {
        ...
        unless (InPhishingWhitelist($linkurl)) {
          ...
          if ($tld && $ttld && $sld && $tsld && $tld eq $ttld && $sld eq $tsld) {
            ...
            if ($Baruwa::Scanner::Config::SecondLevelDomainExists{"$sld.$tld"}) {
              # domain.org.uk
              $TheyMatch = 1 if $third && $tthird && $third eq $tthird;
            } else {
              # Maybe we have a 3rd level domain base?
              if ($Baruwa::Scanner::Config::SecondLevelDomainExists{"$third.$sld.$tld"}) {
          ...
          if ($linkurl ne "") {
            if ($TheyMatch) {
              ...
              if ($numbertrap) {
                print Baruwa::Scanner::Config::LanguageValue(0, 'numericlinkwarning')
                      . ' '
                ...
                use bytes; # Don't send UTF16 to syslog, it breaks!
                Baruwa::Scanner::Log::NoticeLog('Found ip-based phishing fraud from ' .
              ...
              print $possiblefraudstart . ' "' . $linkurl . '" ' .
                    Baruwa::Scanner::Config::LanguageValue(0, 'possiblefraudend') .
                    ' ' if $PhishingHighlight && !$AlreadyReported; # && !InPhishingWhitelist($linkurl);
              $DisarmPhishingFound = 1;
              $linkurl = substr $linkurl, 0, 80;
              $squashedtext = substr $squashedtext, 0, 80;
              $DisarmDoneSomething{'phishing'} = 1 if $PhishingHighlight; #JKF1 $PhishingSubjectTag;
              use bytes; # Don't send UTF16 to syslog, it breaks!
              Baruwa::Scanner::Log::NoticeLog('Found phishing fraud from %s ' .
        ...
        if ($alarm ||
          ...
          unless (InPhishingWhitelist($linkurl)) {
            ...
            $DisarmDoneSomething{'phishing'} = 1 if $PhishingHighlight; #JKF1 $PhishingSubjectTag;
            if ($numbertrap) {
              Baruwa::Scanner::Log::InfoLog('Found ip-based phishing fraud from ' .
                                        '%s in %s', $DisarmLinkURL, $id);
                                        #'%s in %s', $linkurl, $id);
            } else {
              Baruwa::Scanner::Log::InfoLog('Found phishing fraud from %s ' .
                                        'claiming to be %s in %s',
                                        $DisarmLinkURL, $squashedtext, $id);
                                        #$linkurl, $squashedtext, $id);
            }
...
sub HTMLEntityToText {
  ...
  unless ($textfh->open(">$textname")) {
    Baruwa::Scanner::Log::WarnLog('Could not create plain text file %s', $textname);
    return;
  }
...
sub QuarantineDOS {
    ...
    Baruwa::Scanner::Log::WarnLog('Quarantined message %s as it caused Baruwa to crash several times', $message->{id});
    ...
    my $report = Baruwa::Scanner::Config::LanguageValue($message, 'baruwa') . ': ' . Baruwa::Scanner::Config::LanguageValue($message, 'killedbaruwa');
