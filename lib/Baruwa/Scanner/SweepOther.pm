...
package Baruwa::Scanner::SweepOther;
...
sub ScanBatch {
    ...
    $0 = 'Baruwa: scanning for filenames and filetypes';
    ...
    $basefh->open('.')
      or Baruwa::Scanner::Log::DieLog( "Could not opendir $BaseDir, %s", $! );
    ...
    while ( $DirEntry = $basefh->read() ) {
        ...
        if ( -f _ && $DirEntry =~ /\.header$/ ) {
            ...
            next
              unless Baruwa::Scanner::Config::Value( 'dangerscan', $message ) =~
              /1/;
            my @headers = <$headerfh>;
            ...
            if ( grep /^X-Spanska:/i, @headers ) {
                Baruwa::Scanner::Log::NoticeLog(
                    "Other Checks: Found Happy virus in %s", $id );
            ...
            {
                Baruwa::Scanner::Log::NoticeLog(
                    "Other Checks: Found Eudora "
                      . "long-MIME-boundary exploit in %s",
                    $id
                );
                ...
                $message->{entity}->head->mime_attr( "Content-type.boundary" =>
                      "__Baruwa_found_Eudora_long_boundary_attack__" );
                $message->{otherinfected}++;
            }
        ...
        if ( -d _ ) {
            ...
            my $LogNames = Baruwa::Scanner::Config::Value( 'logpermittedfilenames', $message );
            ...
            while ( defined( $safename = $messagefh->read() ) ) {
                ...
                for ($i = 0 ; !$MatchFound && $i < scalar(@$FilenameRules) ; $i++) {
                    ( $allowdeny, $regexp, $iregexp, $logtext, $usertext ) = split( /\0/, $FilenameRules->[$i] );
                    ...
                    if ( $allowdeny =~ /^rename/i ) {
                        ...
                        if ( $entity && $entity->head ) {
                            ...
                            if ( $newname ne '' ) {
                                Baruwa::Scanner::Log::InfoLog("Filename Checks: %s renamed %s to %s", $id, $attach, $newname );

                                 #print STDERR "Renaming $attach to $newname\n";
                                 # These are the only 2 places the user's filename has to change
                                $entity->head->mime_attr("content-disposition.filename", $newname );
                                $entity->head->mime_attr( "content-type.name", $newname );
                        ...
                        my $newto = join( ',', @newto );
                        Baruwa::Scanner::Log::InfoLog("Filetype Checks: Forwarding %s %s to %s", $id, $notypesafename, $newto );
                    } else {
                        Baruwa::Scanner::Log::InfoLog("Filename Checks: Allowing %s %s", $id, $notypesafename ) if $LogNames;
                    }
                }
                Baruwa::Scanner::Log::InfoLog("Filename Checks: Allowing %s %s " . "(no rule matched)", $id, $notypesafename ) if $LogNames && !$MatchFound;
            }
        }
    }
    $basefh->close();
    ...
    return $counter if !Baruwa::Scanner::Config::Value('filecommand');
    return $counter
...
sub CheckFileContentTypes {
    ...
    $Kid         = new FileHandle;
    $TimedOut    = 0;
    $filecommand = Baruwa::Scanner::Config::Value('filecommand');
    ...
    eval {
        ...
        if ($pid) {
            ...
            alarm Baruwa::Scanner::Config::Value('filetimeout');
            ...
            Baruwa::Scanner::Log::WarnLog("Can't run file command " . "(\"$filecommand\"): $!" );
            exit 1;
        }
    };
    alarm 0;    # 2.53
    ...
    Baruwa::Scanner::Log::DebugLog("Completed checking by $filecommand");

    # Catch failures other than the alarm
    Baruwa::Scanner::Log::DieLog("File checker failed with real error: $@")
    ...
    Baruwa::Scanner::Log::WarnLog("File checker $filecommand timed out!")
      if $TimedOut;

    # If we are not using "file -i" at all, then just short-circuit all
    # of this, and check file command output only.
    unless ( $Baruwa::Scanner::Config::UsingFileICommand
        || Baruwa::Scanner::Config::Value('denyfilemimetypes')
        || Baruwa::Scanner::Config::Value('adenyfilemimetypes') )
    {
    ...
    eval {
        ...
        if ($pid) {
            ...
            #exec "$filecommand */*"; # Shouldn't do this like this!
            exec( map { m/(.*)/ } split( / +/, $filecommand ), '-i',
                @filelist );
            Baruwa::Scanner::Log::WarnLog(
                "Can't run 'file -i' command " . "(\"$filecommand -i\"): $!" );
            exit 1;
        }
    };
    alarm 0;    # 2.53
    ...
    # Note to self: I only close the KID in the parent, not in the child.
    Baruwa::Scanner::Log::DebugLog("Completed checking by $filecommand -i");

    # Catch failures other than the alarm
    Baruwa::Scanner::Log::DieLog(
        "File mime-type checker failed with real error: $@")
      if $@ and $@ !~ /Command Timed Out/;
    ...
    # Return failure if the command timed out, otherwise return success
    Baruwa::Scanner::Log::WarnLog("File checker $filecommand -i timed out!")
      if $TimedOut;
...
sub CheckFileTypesRules {
    ...
    while ( ( $id, $attachtypes ) = each %$FileOutput ) {
        ...
        if ( $id ne $lastid ) {
            ...
            $LogTypes = Baruwa::Scanner::Config::Value( 'logpermittedfiletypes', $message );
        ...
        while ( ( $safename, $type ) = each %$attachtypes ) {
            ...
            for ( $i = 0 ; !$MatchFound && $i < @$FiletypeRules ; $i++ ) {
                ...
                next
                  if $regexp eq ''
                  || $regexp eq
                  '-';    # Skip this rule if there's no regexp at all
                ...
                if ( $allowdeny =~ /^rename/i ) {
                    # It's a rename rule, not really a rejection or acceptance.
                    Baruwa::Scanner::Log::InfoLog( "Filename Checks: %s (%s %s)",
                        $logtext, $id, $attach );
                    my $entity = $message->{file2entity}{$attach};
                    ...
                    if ( $entity && $entity->head && $RenamePattern ) {
                        ...
                        Baruwa::Scanner::Log::InfoLog(
                            "Filetype Checks: %s renamed %s to %s",
                            $id, $attach, $newname );
                    ...
                    my $newto = join( ',', @newto );
                    Baruwa::Scanner::Log::InfoLog(
                        "Filetype Checks: Forwarding %s %s to %s",
            ...
            Baruwa::Scanner::Log::InfoLog(
                "Filetype Checks: Allowing %s %s " . "(no match found)",
                $id, $notypesafename )
        ...
        while ( ( $safename, $type ) = each %$attachtypes ) {
            ...
            for ( $i = 0 ; !$MatchFound && $i < @$FiletypeRules ; $i++ ) {
                ...
                if ( $allowdeny =~ /^deny[^@]*$/i ) {
                    ...
                    my $newto = join( ',', @newto );
                    Baruwa::Scanner::Log::InfoLog(
                        "Filetype Mime Checks: Forwarding %s %s to %s",
                        $id, $notypesafename, $newto );
                }
                else {
                    Baruwa::Scanner::Log::InfoLog(
                        "Filetype Mime Checks: Allowing %s %s",
                        $id, $notypesafename )
                      if $LogTypes;
                }
            }
            ...
            # Log it as allowed if it didn't match any rule
            Baruwa::Scanner::Log::InfoLog(
                "Filetype Mime Checks: Allowing %s %s " . "(no match found)",
                $id, $notypesafename )
              if $LogTypes && !$MatchFound;
        }
    }
