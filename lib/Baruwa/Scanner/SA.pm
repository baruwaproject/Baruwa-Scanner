...
sub initialise {
    ...
    unless (Baruwa::Scanner::Config::IsSimpleValue('usespamassassin')
        ...
        $settings{post_config_text} =
          Baruwa::Scanner::ConfigSQL::ReturnSpamAssassinConfig();
        if (defined $settings{post_config_text}) {
            $settings{post_config_text} .=
              Baruwa::Scanner::Config::SpamAssassinPostConfig();
        } else {
            $settings{post_config_text} =
              Baruwa::Scanner::Config::SpamAssassinPostConfig();
        }
        ...
        unless (Baruwa::Scanner::Config::IsSimpleValue('usesacache')
            ...
            $Baruwa::Scanner::SA::cachefilename =
              Baruwa::Scanner::Config::Value("sacache");
            $Baruwa::Scanner::SA::cachedbh =
              DBI->connect("dbi:SQLite:$Baruwa::Scanner::SA::cachefilename",
            ...
            # Speed up writes at the cost of database integrity. Only tmp data!
                ...
                $Baruwa::Scanner::SA::cachedbh->do(
                    "CREATE UNIQUE INDEX md5_uniq ON cache(md5)");
                $Baruwa::Scanner::SA::cachedbh->do(
                    "CREATE INDEX last_seen_idx ON cache(last)");
                $Baruwa::Scanner::SA::cachedbh->do(
                    "CREATE INDEX first_seen_idx ON cache(first)");
                $SQLiteInstalled = 1;
                SetCacheTimes();
                ...
                Baruwa::Scanner::Log::WarnLog(
                    "Could not create SpamAssassin cache database %s",
                    $Baruwa::Scanner::SA::cachefilename);
                $SQLiteInstalled = 0;
                print STDERR
                  "Could not create SpamAssassin cache database $Baruwa::Scanner::SA::cachefilename\n"
                  if $WantLintOnly;
            }
        }
        ...
        $Baruwa::Scanner::SA::BayesRebuildLock =
          Baruwa::Scanner::Config::Value('lockfiledir')
          . '/Baruwa.bayes.rebuild.lock';
        $Baruwa::Scanner::SA::BayesRebuildStartLock =
          Baruwa::Scanner::Config::Value('lockfiledir')
          . '/Baruwa.bayes.starting.lock';
        $Baruwa::Scanner::SA::WaitForRebuild =
          Baruwa::Scanner::Config::Value('bayeswait');
        $Baruwa::Scanner::SA::DoingBayesRebuilds =
          Baruwa::Scanner::Config::Value('bayesrebuild');
        if ($RebuildBayes) {
            ...
            {   flock($RebuildLockH, $LOCK_EX)
                ...
                Baruwa::Scanner::Log::InfoLog(
                    'SpamAssassin Bayes database rebuild starting');
                eval {
                ...
                Baruwa::Scanner::Log::WarnLog(
                    "SpamAssassin Bayes database rebuild "
                      . "failed with error: %s",
                    $@
                ) if $@;
                ...
                $RebuildLockH->close();
                Baruwa::Scanner::Log::InfoLog(
                    'SpamAssassin Bayes database rebuild completed');
            }
...
sub Checks {
    ...
    if ($maxfailures > 0) {
        if ($SAsuccessqsum >= 2 * $maxfailures) {
            ...
            $Baruwa::Scanner::SA::SAspamtest->{conf}->{local_tests_only} = 1;
            Baruwa::Scanner::Log::WarnLog(
                "Disabling SpamAssassin network checks");
        }
    }
    ...
    if ($Baruwa::Scanner::SA::DoingBayesRebuilds) {
        ...
        {   print $RebuildLockH
              "SpamAssassin Bayes database locked for use by " . "Baruwa $$\n";
        ...
        if ($Lockopen) {
            ...
            Baruwa::Scanner::Log::WarnLog(
                "At start of SA checks could not open %s, %s",
                $Baruwa::Scanner::SA::BayesRebuildLock, $!);
        }
    }
    ...
    if ($testcache) {
        if (my $cachehash = CheckCache($md5digest)) {
            ...
            Baruwa::Scanner::Log::InfoLog(
                "SpamAssassin cache hit for message %s",
                $message->{id});
...
sub CheckCache {
    ...
    my ($sql, $sth);
    $sql =
      "SELECT md5, count, last, first, sasaysspam, sahighscoring, sascore, saheader, salongreport FROM cache WHERE md5=?";
    my $hash =
      $Baruwa::Scanner::SA::cachedbh->selectrow_hashref($sql, undef, $md5);
    ...
    if (defined($hash)) {
        ...
        $sql =
          "UPDATE cache SET count=count+1, last=strftime('%s','now') WHERE md5=?";
...
sub CacheResult {
   ...
   #print STDERR "dbh is $dbh and cachedbh is $Baruwa::Scanner::SA::cachedbh\n";
    ...
    my $sql =
      "INSERT INTO cache (md5, count, last, first, sasaysspam, sahighscoring, sascore, saheader, salongreport) VALUES (?,?,?,?,?,?,?,?,?)";
...
sub CacheExpire {
    ...
    Baruwa::Scanner::Log::InfoLog(
        "Expired %s records from the SpamAssassin cache", $rows)
      if $rows > 0;
...
sub SAForkAndTest {
    ...
    if ($pid > 0) {
        $pid2delete = $pid;
        my $maxfailures =
          Baruwa::Scanner::Config::Value('maxspamassassintimeouts');
        ...
        if ($pid2delete && $Baruwa::Scanner::SA::SABayesLock) {
            ...
            if ($lockfh->open($Baruwa::Scanner::SA::SABayesLock)) {
                ...
                if ($pidinlock =~ /$pid2delete/) {
                    unlink $Baruwa::Scanner::SA::SABayesLock;
                    Baruwa::Scanner::Log::InfoLog(
                        "Delete bayes lockfile for %s", $pid2delete);
                }
                $lockfh->close();
            }
        }
