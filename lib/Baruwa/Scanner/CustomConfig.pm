...
#
# For example, say you had /etc/mail/baruwa/spam.bydomain/whitelist/jules.fm
...
# called 'default'.
#
# To enable these functions, set the following in your baruwa.conf file:
...
my $WhitelistDir = '/etc/mail/baruwa/spam.bydomain/whitelist';
my $BlacklistDir = '/etc/mail/baruwa/spam.bydomain/blacklist';
...
sub InitByDomainSpamWhitelist {
    Baruwa::Scanner::Log::InfoLog(
        "Starting up by-domain spam whitelist, " . "reading from %s",
        $WhitelistDir );
    my $domains = CreateByDomainList( $WhitelistDir, \%Whitelist );
    Baruwa::Scanner::Log::InfoLog( "Read whitelist for %d domains", $domains );
}

sub InitByDomainSpamBlacklist {
    Baruwa::Scanner::Log::InfoLog(
        "Starting up by-domain spam blacklist, " . "reading from %s",
        $BlacklistDir );
    my $domains = CreateByDomainList( $BlacklistDir, \%Blacklist );
    Baruwa::Scanner::Log::InfoLog( "Read blacklist for %d domains", $domains );
}
...
sub EndByDomainSpamWhitelist {
    Baruwa::Scanner::Log::InfoLog("Closing down by-domain spam whitelist");
}

sub EndByDomainSpamBlacklist {
    Baruwa::Scanner::Log::InfoLog("Closing down by-domain spam blacklist");
}
...
sub InitSQLLogging {
    Baruwa::Scanner::Log::InfoLog("Initialising SQL Logging temp files");
...
sub EndSQLLogging {
    ...
    my ($dbh);
    $dbh = DBI->connect( "DBI:mysql:baruwa:192.168.0.51",
        "baruwa", "", { 'PrintError' => 0 } )
      or Baruwa::Scanner::Log::DieLog( "Cannot connect to the database: %s",
        $DBI::errstr );

    # Rewind to start of logfile1
    $logfile1->flush();
    seek( $logfile1, 0, 0 )
      or Baruwa::Scanner::Log::DieLog( "EndSQLLogging seek: %s", $! );
    ...
    while (<$logfile1>) {
        ...
        my ($sth) = $dbh->prepare(
"INSERT INTO maillog_mail (time, msg_id, size, from_user, from_domain, subject, clientip, archives, isspam, ishighspam, sascore, spamreport) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)"
        );
        $sth->execute(
          ...
          or Baruwa::Scanner::Log::DieLog( "Cannot insert row: %s", $DBI::errstr );
    }
...
"INSERT INTO maillog_report (msg_id, filename, filereport) VALUES (?,?,?)"
        ...
        $sth->execute( $fields[0], $fields[1], $fields[2] )
          or Baruwa::Scanner::Log::DieLog( "Cannot insert row: %s", $DBI::errstr );
    }
...
"INSERT INTO maillog_recipient (msg_id, to_user, to_domain) VALUES (?,?,?)"
    ...
    $logfile2->close();
    $logfile3->close();
    Baruwa::Scanner::Log::InfoLog("Database flush completed");
}
...
sub SQLLogging {
    ...
    print $logfile1 join( "\t",
        $timestamp,   $id,         $size,     $from_user,
        $from_domain, $subject,    $clientip, $archives,
        $isspam,      $ishighspam, $sascore,  $spamreport )
      . "\n";
...
# in your baruwa.conf file, having added this code to
#    /usr/share/baruwa/CustomFunctions/CustomConfig.pm
#
# It uses a configuration file whose path is
my ($InternalAccountList) = '/etc/mail/baruwa/internal.accounts.conf';
...
# defined in the baruwa.conf file.
#
...
sub InitInternalActions {
    Baruwa::Scanner::Log::InfoLog("Initialising Internal account list");
    my $listfile = new FileHandle;

    unless ( $listfile->open("<$InternalAccountList") ) {
        Baruwa::Scanner::Log::WarnLog(
            "Could not read list of internal accounts " . "from %s",
            $InternalAccountList );
        return;
    }
    ...
    while (<$listfile>) {
        ...
        if ( $keyword =~ /domain/i ) {
            ...
            Baruwa::Scanner::Log::WarnLog( "Syntax error in %s at line %d",
                $InternalAccountList, $line );
        }
    }
    $listfile->close();
    Baruwa::Scanner::Log::InfoLog(
        "Internal Account List read %d domains and %d " . "accounts",
        $domains, $accounts );
}
...
sub EndInternalActions {
    ...
    # No shutdown code needed here at all.
    Baruwa::Scanner::Log::InfoLog("Shutting down internal accounts list");
}
...
sub InternalActions {
    ...
    foreach $todomain ( @{ $message->{todomain} } ) {
        ...
        unless ( $InternalDomains{$todomain} ) {
            Baruwa::Scanner::Log::WarnLog(
                "Internal-only account %s attempted to "
                  . "send mail to external address \@%s",
                $fromac, $todomain
            );
...
#
# To use this code from baruwa.conf, set this in baruwa.conf:
...
#
# The config file is stored in /etc/mail/baruwa/IPBlock.conf.  You
...
my $WhitelistFile = '/etc/mail/baruwa/IPBlock.conf';
my $LockFile      = '/var/spool/baruwa/IPBlock.lock';

#my $LogFile     = '/var/spool/baruwa/IPBlock.log';
my $BlockDB                   = '/var/spool/baruwa/IPBlock.db';
...
$my_mta = lc( Baruwa::Scanner::Config::QuickPeek( $ConfFile, 'mta' ) )
  if $ConfFile
  && -f $ConfFile;
if ( $my_mta eq "postfix" ) {
    $AccessDB = "/etc/postfix/access.db";
    $Refusal  = "$FailCode Site blocked by Baruwa due to excessive email";

}
elsif ( $my_mta eq "sendmail" ) {
    $AccessDB = "/etc/mail/access.db";
    $Refusal =
      "\"$FailCode Site blocked by Baruwa due to excessive email\"";
}
else {
    $AccessDB = "";
    $Refusal  = "";
...
sub InitIPBlock {
    ...
    if ( $my_mta !~ /postfix|sendmail/ ) {
        Baruwa::Scanner::Log::WarnLog(
            "IPBlock Currently not supported " . "for your MTA %s", $my_mta );
        return 1;
    }

    Baruwa::Scanner::Log::InfoLog("Initialising IP blocking");
    $LimitsH = new FileHandle;
    unless ( $LimitsH->open($WhitelistFile) ) {
        Baruwa::Scanner::Log::WarnLog(
            "Could not read IPBlock configuration from %s",
            $WhitelistFile );
        return;
    }
    $counter = 0;
    ...
    close($LimitsH);
    Baruwa::Scanner::Log::InfoLog( "Read %d IP blocking entries from %s",
        $counter, $WhitelistFile );
}

sub EndIPBlock {
    Baruwa::Scanner::Log::InfoLog("Closing down IP blocking");
}
...
sub IPBlock {
  ...
  #  Baruwa::Scanner::Log::WarnLog("IPBlock: Cannot open %s for writing", $LogFile);
  #  return 1;
  #}
  #print $LogH $ip . "\n";
  #$LogH->close;
    ...
    unless ( tie %BlockDB, "AnyDBM_File", $BlockDB, O_RDWR | O_CREAT, 0644 ) {
        Baruwa::Scanner::Log::WarnLog( "IPBlock: Could not open/create %s",
            $BlockDB );
...
#print STDERR "Found IP in BlockDB, counter=$counter time=$time done=$donealready\n";
    ...
    if ( $counter > $MaxMessagesPerHour && !$donealready ) {
        ...
        unless ( tie %AccessDB, "AnyDBM_File", $AccessDB, O_RDWR, 0644 ) {
            Baruwa::Scanner::Log::WarnLog(
                "IPBlock: Could not open access database %s", $AccessDB );
        ...
        Baruwa::Scanner::Log::NoticeLog( "IPBlock: Adding block for %s", $ip );
        $AccessDB{$ip}       = $Refusal;
        $AccessDB{$hostname} = $Refusal;
        $donealready =
          1;    # Mark it so we don't try to waste CPU setting it twice
        untie %AccessDB;
    }
...
sub IPBlockIP2Hostname {
    ...
    unless ( $pipe = new IO::Pipe ) {
        Baruwa::Scanner::Log::WarnLog( 'IPBlock: Could not create pipe, %s', $! );
        return "";
    }
    ...
    my $pid = fork();
    unless ( defined($pid) ) {
        Baruwa::Scanner::Log::WarnLog( 'IPBlock: Could not fork process, %s', $! );
        return "";
    }
    ...
    Baruwa::Scanner::Log::WarnLog(
        "IPBlock: Hostname lookup for %s failed with real error: %s",
        $IPstring, $@ )
    ...
    if ( $pid > 0 ) {
        Baruwa::Scanner::Log::WarnLog(
            "IPBlock: Hostname lookup for %s timed out and was killed",
            $IPstring );
...
# in /etc/mail/baruwa/baruwascanner.conf
# and put the rules in these 2 files:
my $Spamlistfile       = '/etc/mail/baruwa/rules/spam.list.rules';
my $Spamdomainlistfile = '/etc/mail/baruwa/rules/spam.domain.list.rules';

my $Codebasefile   = '/etc/mail/baruwa/rules/codebase.tags.rules';
my $Externalfile   = '/etc/mail/baruwa/rules/external.message.bodies.rules';
my $Filenamefile   = '/etc/mail/baruwa/rules/filename.rules';
my $Htmltotextfile = '/etc/mail/baruwa/rules/html.to.text.rules';
my $Iframefile     = '/etc/mail/baruwa/rules/iframe.tags.rules';
my $Partialfile    = '/etc/mail/baruwa/rules/partial.message.rules';

use FileHandle;
my (
    %Fastspamlist, %Fastspamdomainlist, %Fastcodebase, %Fastexternal,
...
sub InitFastSpamList {
    ...
    unless ( $fh->open($Spamlistfile) ) {
        Baruwa::Scanner::Log::WarnLog(
            "Could not read fast spam list rules from %s",
            $Spamlistfile );
        return;
    }
    ...
    Baruwa::Scanner::Log::InfoLog( "Read %d fast spam list rules", $counter );
...
sub InitFastSpamDomainList {
    ...
    unless ( $fh->open($Spamdomainlistfile) ) {
        Baruwa::Scanner::Log::WarnLog(
            "Could not read fast spam domain list rules " . "from %s",
            $Spamdomainlistfile );
        return;
    }
    ...
    Baruwa::Scanner::Log::InfoLog( "Read %d fast spam domain list rules",
        $counter );
...
sub InitFastCodebase {
    ...
    unless ( $fh->open($Codebasefile) ) {
        Baruwa::Scanner::Log::WarnLog(
            "Could not read fast codebase tags rules " . "from %s",
            $Codebasefile );
        return;
    }
    ...
    Baruwa::Scanner::Log::InfoLog( "Read %d fast codebase tags rules", $counter );
...
sub InitFastExternal {
    ...
    unless ( $fh->open($Externalfile) ) {
        Baruwa::Scanner::Log::WarnLog(
            "Could not read fast external bodies rules " . "from %s",
            $Externalfile );
        return;
    }
    ...
    Baruwa::Scanner::Log::InfoLog( "Read %d fast external bodies rules", $counter );
...
sub InitFastFilename {
    ...
    unless ( $fh->open($Filenamefile) ) {
        Baruwa::Scanner::Log::WarnLog(
            "Could not read fast filename-rules rules " . "from %s",
            $Filenamefile );
        return;
    }
    ...
    Baruwa::Scanner::Log::InfoLog( "Read %d fast filename-rules rules", $counter );
...
sub InitFastHtmlToText {
    ...
    unless ( $fh->open($Htmltotextfile) ) {
        Baruwa::Scanner::Log::WarnLog(
            "Could not read fast HTML To Text rules " . "from %s",
            $Htmltotextfile );
        return;
    }
    ...
    Baruwa::Scanner::Log::InfoLog( "Read %d fast HTML To Text rules", $counter );
...
sub InitFastIframe {
    ...
    unless ( $fh->open($Iframefile) ) {
        Baruwa::Scanner::Log::WarnLog(
            "Could not read fast IFrame Tags rules " . "from %s", $Iframefile );
        return;
    }
    ...
    Baruwa::Scanner::Log::InfoLog( "Read %d fast IFrame Tags rules", $counter );
...
sub InitFastPartial {
    ...
    unless ( $fh->open($Partialfile) ) {
        Baruwa::Scanner::Log::WarnLog(
            "Could not read fast partial message rules " . "from %s",
            $Partialfile );
        return;
    }
    ...
    Baruwa::Scanner::Log::InfoLog( "Read %d fast partial message rules", $counter );
...
sub EndFastSpamList {
    ...
    # No shutdown code needed here at all.
    Baruwa::Scanner::Log::InfoLog("Ending SpamList");
}

sub EndFastSpamDomainList {

    # No shutdown code needed here at all.
    Baruwa::Scanner::Log::InfoLog("Ending SpamDomainList");
}

sub EndFastCodebase {

    # No shutdown code needed here at all.
    Baruwa::Scanner::Log::InfoLog("Ending Codebase");
}

sub EndFastExternal {

    # No shutdown code needed here at all.
    Baruwa::Scanner::Log::InfoLog("Ending External");
}

sub EndFastFilename {

    # No shutdown code needed here at all.
    Baruwa::Scanner::Log::InfoLog("Ending Filename");
}

sub EndFastHtmlToText {

    # No shutdown code needed here at all.
    Baruwa::Scanner::Log::InfoLog("Ending HtmlToText");
}

sub EndFastIframe {

    # No shutdown code needed here at all.
    Baruwa::Scanner::Log::InfoLog("Ending Iframe");
}

sub EndFastPartial {

    # No shutdown code needed here at all.
    Baruwa::Scanner::Log::InfoLog("Ending Partial");
}
...
push @INC,"/usr/share/Baruwa","/usr/share/Baruwa";
use FileHandle;
use Fcntl qw(:DEFAULT :flock);
use Sys::Syslog;
BEGIN { @AnyDBM_File::ISA = qw(DB_File GDBM_File NDBM_File SDBM_File) }
use AnyDBM_File;
require Baruwa::Scanner::Config;
use strict 'vars';
use strict 'refs';
no  strict 'subs'; # Allow bare words for parameter %'s
...
my $ConfFile = $ARGV[0];
die "Usage : <scriptname> /path/to/baruwa.conf\n" if($ConfFile eq "");
...
#my $Refusal      = '"451 Site blocked by Baruwa due to excessive email"';
