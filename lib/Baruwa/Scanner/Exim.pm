...
my @SendmailOptions = qw"-t -oi -oem -F Baruwa -f";
...
# baruwa disposition headers with the "right" one,
            ...
            && $HitLimit1 + $HitLimit2 + $HitLimit3 + $HitLimit4 < 1 )
        {
           ...
           # If they want a particular message id, ignore it if it doesn't match
                ...
                /1/ )
            {
                ...
                $HitLimit1 = 1 if $CleanMsgs >= $MaxCleanM;
                $HitLimit2 = 1 if $CleanBytes >= $MaxCleanB;
            }
        }
