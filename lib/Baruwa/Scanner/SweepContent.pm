...
package Baruwa::Scanner::SweepContent;
...
sub ScanBatch {
    ...
    $0 = 'Baruwa: dangerous content scanning';
    while ( ( $id, $message ) = each %{ $batch->{messages} } ) {
        ...
        if ( ( !Baruwa::Scanner::Config::Value( 'allowpartial', $message ) )
            && FindPartialMessage( $message, $ent ) )
        {
            $counter++;
            Baruwa::Scanner::Log::WarnLog(
                'Content Checks: Detected and rejected '
                  . 'fragmented message section in %s',
                $id
            );
        }
        ...
        if ( ( !Baruwa::Scanner::Config::Value( 'allowexternal', $message ) )
            && FindExternalBody( $message, $ent ) )
        {
            $counter++;
            Baruwa::Scanner::Log::WarnLog(
                'Content Checks: Detected and rejected '
                  . 'external message body in %s',
                $id
            );
        }
        ...
        my $maxmessagesize = Baruwa::Scanner::Config::Value( 'maxmessagesize', $message );
        $message->{maxmessagesize} = $maxmessagesize;   # Store it for reporting
        if ( $maxmessagesize > 0 && $message->{size} > $maxmessagesize ) {
            Baruwa::Scanner::Log::WarnLog(
                "Content Checks: Message %s is bigger than " . "%d bytes",
                $message->{id}, $maxmessagesize );
            $message->{otherreports}{""} .=
                Baruwa::Scanner::Config::LanguageValue( $message, 'toobig' ) . ": "
              . $message->{size}
              . " bytes\n";
        ...
        my $iframevalue = Baruwa::Scanner::Config::Value( 'allowiframetags', $message );
        my $objectvalue = Baruwa::Scanner::Config::Value( 'allowobjecttags', $message );
        my $formvalue = Baruwa::Scanner::Config::Value( 'allowformtags', $message );
        my $scriptvalue = Baruwa::Scanner::Config::Value( 'allowscripttags', $message );
        my $webbugvalue = Baruwa::Scanner::Config::Value( 'allowwebbugtags', $message );
        my $phishingvalue = Baruwa::Scanner::Config::Value( 'findphishing', $message );
        my $allowiframes   = 0;
        my $allowobjects   = 0;
        my $allowforms     = 0;
        my $allowscripts   = 0;
        ...
        $convertiframes = 1
          if $iframevalue !~ /0/ && $iframevalue =~ /convert/i;
        $convertobjects = 1
          if $objectvalue !~ /0/ && $objectvalue =~ /convert/i;
        $convertforms = 1 if $formvalue !~ /0/ && $formvalue =~ /convert/i;
        $convertscripts = 1
          if $scriptvalue !~ /0/ && $scriptvalue =~ /convert/i;
        $convertwebbugs = 1
          if $webbugvalue !~ /0/ && $webbugvalue =~ /convert/i;
        $stripdangerous =
          Baruwa::Scanner::Config::Value( 'stripdangeroustags', $message );
        ...
        if (
            ...
            && FindHTMLExploits(
                ...
                $allowscripts,   $allowwebbugs, $convertiframes,
                $convertobjects, $convertforms, $convertscripts,
                $convertwebbugs, $stripdangerous
            )
          )
        {
            $counter++;
            Baruwa::Scanner::Log::WarnLog(
                'Content Checks: Detected HTML-' . 'specific exploits in %s',
                $id );
        }
        ...
        if ( $encrypted
            && Baruwa::Scanner::Config::Value( 'blockencrypted', $message ) =~ /1/ )
        {
            $reason = 'encrypted';
        }
        if ( !$encrypted
            && Baruwa::Scanner::Config::Value( 'blockunencrypted', $message ) =~
            /1/ )
        {
            $reason = 'unencrypted';
        }
        if ($reason) {
            $message->{otherreports}{""} .=
              Baruwa::Scanner::Config::LanguageValue( $message, $reason ) . "\n";
            $message->{othertypes}{""} .= "e";

            #$message->{otherinfected}++;
            $counter++;
            Baruwa::Scanner::Log::WarnLog(
                'Content Checks: Detected and blocked ' . '%s message in %s',
                $reason, $id );
        }
        ...
        #  if Baruwa::Scanner::Config::Value('archivepublickeys', $message);
        ...
        if ( Baruwa::Scanner::Config::Value( 'htmltotext', $message ) =~ /1/ ) {
    ...
    Baruwa::Scanner::Log::InfoLog(
        'Content Checks: Need to convert HTML to plain '
          . 'text in %s messages',
        $stripcounter
    ) if $stripcounter > 0;
    return $counter;
...
sub CheckAttachmentSizes {
    ...
    $maxsize = Baruwa::Scanner::Config::Value( 'maxattachmentsize', $message );
    $minsize = Baruwa::Scanner::Config::Value( 'minattachmentsize', $message );
    ...
    $basefh->open('.')
      or Baruwa::Scanner::Log::DieLog( "Could not open attachment dir %s, %s",
        $BaseDir, $! );
    $counter = 0;
    while ( $safename = $basefh->read() ) {
        ...
        if ( $maxsize >= 0 && $attachsize > $maxsize ) {
            ...
            Baruwa::Scanner::Log::NoticeLog(
                "Attachment size check: %s > %s (%s) in %s",
    ...
    if ( $type && $type =~ /message\/partial/i ) {
        ...
        if ($bodypath) {
            $message->{otherreports}{$bodypath} .=
              Baruwa::Scanner::Config::LanguageValue( $message, 'partialmessage' )
              . "\n";
...
sub LastEntity {
    ...
    if ( $entity && !$entity->bodyhandle ) {
        ...
        return LastEntity( $message, $entity->parts( $NumParts - 1 ) )
          if $NumParts > 0;
...
sub FindHTMLExploits {
    my (
        $message,        $id,             $entity,       $allowiframes,
        $allowobjects,   $allowforms,     $allowscripts, $allowwebbugs,
    ...
    {
        $counter += SearchHTMLBody(
            $message,                  $id,
            $entity->bodyhandle->path, $allowiframes,
            $allowobjects,             $allowforms,
            $allowscripts,             $allowwebbugs,
            $convertiframes,           $convertobjects,
            $convertforms,             $convertscripts,
            $convertwebbugs,           $stripdangerous
        );
    }
...
sub SearchHTMLBody {
    my (
        $message,        $id,             $filename,     $allowiframes,
        $allowobjects,   $allowforms,     $allowscripts, $allowwebbugs,
        $convertiframes, $convertobjects, $convertforms, $convertscripts,
        $convertwebbugs, $stripdangerous
    ) = @_;
    ...
    if ( $fh->open("$filename") ) {
        ...
        my $loggingtags = Baruwa::Scanner::Config::Value( 'loghtmltags', $message );
        ...
        if ( $phishingfound
            && Baruwa::Scanner::Config::Value( 'findphishing', $message ) )
        {
            # Log the <A>
            Baruwa::Scanner::Log::InfoLog( "<A> tag found in message %s from %s",
                $id, $message->{from} )
              if $loggingtags;
        ...
        if ($iframefound) {
            ...
            if ($allowiframes) {
                ...
                $message->{otherreports}{"$attach"} .=
                  Baruwa::Scanner::Config::LanguageValue( $message, 'foundiframe' )
                  . "\n";
                $message->{othertypes}{"$attach"} .= "c";
                $message->{otherinfected}++;
                $message->{silent} = 1 if $silentviruses =~ /\sHTML-IFrame\s/i;
                $counter++;
            }
        }
        if ($formfound) {
            ## Log the <Form>
            Baruwa::Scanner::Log::NoticeLog(
                "HTML-Form tag found in message %s from %s",
                $id, $message->{from} )
              if $loggingtags;
            ...
            if ($allowforms) {
                ...
                $message->{otherreports}{"$attach"} .=
                  Baruwa::Scanner::Config::LanguageValue( $message, 'foundform' )
                  . "\n";
                $message->{othertypes}{"$attach"} .= "c";
                $message->{otherinfected}++;
                $message->{silent} = 1 if $silentviruses =~ /\sHTML-Form\s/i;
                $counter++;
            }
        }
        if ($scriptfound) {
            ## Log the <script>
            Baruwa::Scanner::Log::NoticeLog(
                "HTML-Script tag found in message %s from %s",
                $id, $message->{from} )
              if $loggingtags;
            ...
            if ($allowscripts) {
                ...
                $message->{otherreports}{"$attach"} .=
                  Baruwa::Scanner::Config::LanguageValue( $message, 'foundscript' )
                  . "\n";
                $message->{othertypes}{"$attach"} .= "c";
                $message->{otherinfected}++;
                $message->{silent} = 1 if $silentviruses =~ /\sHTML-Script\s/i;
                $counter++;
            }
        }
        if ($webbugfound) {
            ## Log the <img>
            Baruwa::Scanner::Log::NoticeLog(
                "HTML Img tag found in message %s from %s",
                $id, $message->{from} )
              if $loggingtags;

            #  if Baruwa::Scanner::Config::Value('logwebbugs', $message);
            # Mark the message
            if ($allowwebbugs) {
                ...
                $message->{otherreports}{"$attach"} .=
                  Baruwa::Scanner::Config::LanguageValue( $message, 'foundwebbug' )
                  . "\n";
                $message->{othertypes}{"$attach"} .= "c";
                $message->{otherinfected}++;
                $message->{silent} = 1 if $silentviruses =~ /\sHTML-WebBug\s/i;
                $counter++;
                1;
            }
        }
        if ($codebasefound) {
            Baruwa::Scanner::Log::NoticeLog(
                "HTML-Object tag found in message %s from %s",
                $id, $message->{from} )
              if $loggingtags;
            if ($allowobjects) {
                if ($stripdangerous) {
                ...
                # Mark the message
                $message->{otherreports}{"$attach"} .=
                  Baruwa::Scanner::Config::LanguageValue( $message, 'foundobject' )
                  . "\n";
                $message->{othertypes}{"$attach"} .= "c";
                $message->{otherinfected}++;
                $message->{silent} = 1 if $silentviruses =~ /\sHTML-Codebase\s/i;
                $counter++;
            }
        }
    }
        ...
        Baruwa::Scanner::Log::WarnLog(
            "Could not search \"%s\" in message %s for " . "dangerous HTML",
            $filename, $id );
        $counter++;
    }
...
sub FindExternalBody {
    ...
    if ( $type && $type =~ /message\/external-body/i ) {
        ...
        $message->{entityreports}{$entity} .=
          Baruwa::Scanner::Config::LanguageValue( $message, 'externalbody' ) . "\n";
        $message->{otherinfected}++;
        $counter++;
    }
...
sub SavePublicKey {
    ...
    $keyfilename =
        Baruwa::Scanner::Config::Value( 'publickeyarchivedir', $message ) . '/'
      . $date . '-'
      . $from;
    ...
    unless ( $keyfh->open(">$keyfilename") ) {
        Baruwa::Scanner::Log::WarnLog( 'Could not create public key file %s',
            $keyfilename );
        return;
    }
...
sub FixSubstringBoundaries {
    ...
    foreach $firstlevel (@toplevel) {
        ...
        $root->head->mime_attr( "Content-type.boundary" =>
              "__Baruwa_found_Cyrus_boundary_substring_problem__" );
    ...
    if ($changedit) {
        Baruwa::Scanner::Log::WarnLog(
            'Content Checks: Fixed awkward MIME boundary '
              . 'for Cyrus IMAP server in %s',
            $id
        );
        $message->{bodymodified} = 1;
    }
